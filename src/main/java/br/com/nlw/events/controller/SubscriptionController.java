...
public class SubscriptionController {
    ...
    public ResponseEntity<?> createSubscription(@PathVariable String prettyName, @RequestBody User subscriber, @PathVariable(required=false) Integer userId) {
        try {
            SubscriptionResponse resultado = service.createNewSubscription(prettyName, subscriber, userId);

            if (resultado != null) {
                return ResponseEntity.ok().body(resultado);
            }
            
        }  catch (EventNotFoundException | SubscriptionConflictException | UserIndicadorNotFound e) {
